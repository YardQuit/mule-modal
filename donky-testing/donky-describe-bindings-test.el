;;; donky-describe-bindings-test.el --- Tests for donky-describe-bindings -*- lexical-binding: t; -*-
...
(defun donky-describe-bindings-test--simple-map ()
...
(defun donky-describe-bindings-test--nested-map ()
...
(defun donky-describe-bindings-test--menu-item-map ()
...
(defun donky-describe-bindings-test--complex-def-map ()
...
;; ===========================================================================
;; Section: donky--desc-bindings-collect-leaves
;; Selector: (ert "donky-describe-bindings-collect-leaves")
;; ===========================================================================

;;; --- Basic collection ---

(ert-deftest donky-describe-bindings-collect-leaves-empty-map ()
  "An empty keymap produces no leaf entries.
Expected: nil."
  (should (null (donky--desc-bindings-collect-leaves
                 (make-sparse-keymap) ""))))

(ert-deftest donky-describe-bindings-collect-leaves-nil-def-skipped ()
  "Bindings whose definition is nil are silently ignored.
Expected: nil (empty list)."
  (let ((map (make-sparse-keymap)))
    (define-key map [?a] nil)
    (should (null (donky--desc-bindings-collect-leaves map "")))))

(ert-deftest donky-describe-bindings-collect-leaves-single-level ()
  "A flat keymap yields one entry per binding with correct key strings.
Expected: two entries (\"a\" . #'ignore) and (\"b\" . #'forward-char)."
  (let* ((map (donky-describe-bindings-test--simple-map))
         (res (donky--desc-bindings-collect-leaves map "")))
    (should (= (length res) 2))
...
(ert-deftest donky-describe-bindings-collect-leaves-nested-descends ()
  "Sub-keymaps are traversed recursively.
Expected: top-level 'x' and nested 'SPC q' both present."
  (let* ((map (donky-describe-bindings-test--nested-map))
         (res (donky--desc-bindings-collect-leaves map "")))
    (should (= (length res) 2))
    (should (assoc "x"     res))
...
(ert-deftest donky-describe-bindings-collect-leaves-prefix-accumulated ()
...
Expected: keys from nested maps carry the parent prefix followed by a space."
  (let ((map (make-sparse-keymap))
    ...
    (let ((res (donky--desc-bindings-collect-leaves map "P")))
      (should (assoc "Pg a" res)))))

;;; --- Remap filtering ---

(ert-deftest donky-describe-bindings-collect-leaves-skips-self-insert-remap ()
  "Remap entries targeting `self-insert-command' are excluded.
Expected: empty result (the only binding was such a remap)."
  (let ((res (donky--desc-bindings-collect-leaves
              (donky-describe-bindings-test--remap-map) "")))
    (should (null res))))

(ert-deftest donky-describe-bindings-collect-leaves-non-self-insert-remap-collected ()
...
Expected: one entry whose value is #'ignore."
  (let* ((map (donky-describe-bindings-test--non-sic-remap-map))
         (res (donky--desc-bindings-collect-leaves map "")))
    (should (= (length res) 1))
...
(ert-deftest donky-describe-bindings-collect-leaves-cons-cdr-keymap ()
...
Expected: the inner binding 'j' appears with prefix 'm '."
  (let* ((map (donky-describe-bindings-test--cons-cdr-keymap-map))
         (res (donky--desc-bindings-collect-leaves map "")))
...
(ert-deftest donky-describe-bindings-collect-leaves-menu-item-as-leaf ()
...
Expected: one entry whose definition car is 'menu-item."
  (let* ((map (donky-describe-bindings-test--menu-item-map))
         (res (donky--desc-bindings-collect-leaves map "")))
    (should (= (length res) 1))
...
(ert-deftest donky-describe-bindings-collect-leaves-complex-def-as-leaf ()
  "Non-symbol, non-keymap definitions are stored verbatim as leaves.
Expected: one entry whose cdr is the list (\"my-data\")."
  (let* ((map (donky-describe-bindings-test--complex-def-map))
         (res (donky--desc-bindings-collect-leaves map "")))
    (should (= (length res) 1))
...
(ert-deftest donky-describe-bindings-collect-leaves-order-is-stable ()
...
(ert-deftest donky-describe-bindings-collect-leaves-three-level-nesting ()
...
Expected: deepest binding key is 'a b c'."
  (let ((l1 (make-sparse-keymap))
    ...
    (define-key l2 "b" l3)
    (define-key l1 "a" l2)
    (let ((res (donky--desc-bindings-collect-leaves l1 "")))
      (should (= (length res) 1))
      (should (equal (caar res) "a b c")))))

;;; --- Mixed map ---

(ert-deftest donky-describe-bindings-collect-leaves-mixed-leaf-and-submap ()
...
Expected: three entries — 'p', 's a', 's b'."
  (let ((root (make-sparse-keymap))
    ...
    (let ((res (donky--desc-bindings-collect-leaves root "")))
      (should (= (length res) 3))
      (should (assoc "p"   res))
...
;; ===========================================================================
;; Section: donky--binding-group-name
;; Selector: (ert "donky-describe-bindings-group-name")
;; ===========================================================================

(ert-deftest donky-describe-bindings-group-name-single ()
  "Prefix \"single\" maps to \"Single Keys\".
Expected: \"Single Keys\"."
  (should (equal (donky--binding-group-name "single") "Single Keys")))

(ert-deftest donky-describe-bindings-group-name-g ()
  "Prefix \"g\" maps to \"Goto / Scroll\".
Expected: \"Goto / Scroll\"."
  (should (equal (donky--binding-group-name "g") "Goto / Scroll")))

(ert-deftest donky-describe-bindings-group-name-m ()
  "Prefix \"m\" maps to \"Mark Objects\".
Expected: \"Mark Objects\"."
  (should (equal (donky--binding-group-name "m") "Mark Objects")))

(ert-deftest donky-describe-bindings-group-name-r ()
  "Prefix \"r\" maps to \"Search / Replace\".
Expected: \"Search / Replace\"."
  (should (equal (donky--binding-group-name "r") "Search / Replace")))

(ert-deftest donky-describe-bindings-group-name-z ()
  "Prefix \"z\" maps to \"Scroll\".
Expected: \"Scroll\"."
  (should (equal (donky--binding-group-name "z") "Scroll")))

(ert-deftest donky-describe-bindings-group-name-unknown-prefix ()
  "Unknown single-char prefix is uppercased and suffixed with \" Prefix\".
Expected: \"X Prefix\"."
  (should (equal (donky--binding-group-name "x") "X Prefix")))

(ert-deftest donky-describe-bindings-group-name-multi-char-prefix ()
  "Multi-character unknown prefixes are uppercased in full.
Expected: \"SPC Prefix\"."
  (should (equal (donky--binding-group-name "SPC") "SPC Prefix")))

(ert-deftest donky-describe-bindings-group-name-empty-string ()
  "Empty-string prefix yields \" Prefix\" (leading space).
Documents existing behaviour rather than asserting it is ideal.
Expected: \" Prefix\"."
  (should (equal (donky--binding-group-name "") " Prefix")))

(ert-deftest donky-describe-bindings-group-name-numeric-string ()
  "Numeric string prefix is treated as any unknown prefix.
Expected: \"1 Prefix\"."
  (should (equal (donky--binding-group-name "1") "1 Prefix")))

;; ===========================================================================
;; Section: donky-describe-bindings
;; Selector: (ert "donky-describe-bindings")
;;           runs ALL tests in this file
;;
;;           (ert "donky-describe-bindings-")
;;           runs only this section (avoids matching the sub-section prefixes)
;; ===========================================================================

(defconst donky-describe-bindings-test--expected-title "DONKY Normal Mode Key Bindings"
  "Title string expected in the *DONKY Bindings* buffer.")

;;; --- Pre-condition error ---

(ert-deftest donky-describe-bindings-errors-without-map ()
  "Calling `donky-describe-bindings' when `donky-normal-mode-map' is unbound
raises a `user-error'.
Expected: signal of type `user-error'."
  (let ((had-map (boundp 'donky-normal-mode-map))
        (old-val (and (boundp 'donky-normal-mode-map)
                      (default-value 'donky-normal-mode-map))))
    (when had-map
      (makunbound 'donky-normal-mode-map))
    (unwind-protect
        (should-error (donky-describe-bindings) :type 'user-error)
      (when had-map
        (set-default 'donky-normal-mode-map old-val)))))

;;; --- Buffer creation and basic content ---

(ert-deftest donky-describe-bindings-creates-buffer ()
  "Creates the buffer named *DONKY Bindings*.
Expected: buffer exists after the call."
  (let ((donky-normal-mode-map (donky-describe-bindings-test--simple-map)))
    (when (get-buffer "*DONKY Bindings*")
      (kill-buffer "*DONKY Bindings*"))
    (cl-letf (((symbol-function 'display-buffer) #'ignore))
      (donky-describe-bindings))
    (should (get-buffer "*DONKY Bindings*"))
    (kill-buffer "*DONKY Bindings*")))

(ert-deftest donky-describe-bindings-title-present ()
  "Buffer contains the expected title text.
Expected: first line includes \"DONKY Normal Mode Key Bindings\"."
  (let ((donky-normal-mode-map (donky-describe-bindings-test--simple-map)))
    (cl-letf (((symbol-function 'display-buffer) #'ignore))
      (donky-describe-bindings))
    (with-current-buffer "*DONKY Bindings*"
      (goto-char (point-min))
      (should (search-forward donky-describe-bindings-test--expected-title nil t)))
    (kill-buffer "*DONKY Bindings*")))

(ert-deftest donky-describe-bindings-read-only ()
...
(ert-deftest donky-describe-bindings-truncate-lines ()
...
(ert-deftest donky-describe-bindings-sorted-alphabetically ()
...
(ert-deftest donky-describe-bindings-symbol-leaf-as-button ()
...
(ert-deftest donky-describe-bindings-complex-def-shown-as-text ()
  "Non-symbol definitions render as literal \"[complex]\" text.
Expected: the string \"[complex]\" appears in the buffer."
  (let ((donky-normal-mode-map (donky-describe-bindings-test--complex-def-map)))
    (cl-letf (((symbol-function 'display-buffer) #'ignore))
      (donky-describe-bindings))
    (with-current-buffer "*DONKY Bindings*"
      (goto-char (point-min))
...
(ert-deftest donky-describe-bindings-local-keymap-q-binds-quit-window ()
...
(ert-deftest donky-describe-bindings-local-keymap-ret-binds-push-button ()
...
(ert-deftest donky-describe-bindings-footer-present ()
...
(ert-deftest donky-describe-bindings-point-at-min ()
...
(ert-deftest donky-describe-bindings-overwrite-on-repeat ()
...
Expected: after the second call, only keys from the second map remain."
  (let ((map-a (make-sparse-keymap))
    ...
    (cl-letf (((symbol-function 'display-buffer) #'ignore))
      (let ((donky-normal-mode-map map-a))
        (donky-describe-bindings))
      (with-current-buffer "*DONKY Bindings*"
        (should (search-forward "ignore" nil t)))
      (let ((donky-normal-mode-map map-b))
        (donky-describe-bindings))
      (with-current-buffer "*DONKY Bindings*"
        (goto-char (point-min))
...
;;; donky-describe-bindings-test.el ends here
