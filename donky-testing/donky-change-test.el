;;; donky-change-test.el --- Tests for donky-change -*- lexical-binding: t; -*-
...
;; ===========================================================================
;; Section: donky-change
;; Selector: (ert "donky-change")
;;           runs ALL tests in this file
;; ===========================================================================

;;; --- No region: deletes single char ---

(ert-deftest donky-change-no-region-deletes-single-char ()
...
(ert-deftest donky-change-no-region-enters-insert ()
  "Without region, enters insert mode after deletion.
    Expected: donky-enter-insert called exactly once."
  (let ((call-count 0))
    (with-temp-buffer
      (insert "hello\n")
      (goto-char 1)
...
(ert-deftest donky-change-no-region-from-middle ()
...
(ert-deftest donky-change-no-region-last-char ()
...
(ert-deftest donky-change-no-region-empty-buffer-errors ()
  ...
  (with-temp-buffer
    (cl-letf (((symbol-function 'use-region-p)
              ...
              ((symbol-function 'donky-enter-insert)
               (lambda () nil)))
      (should-error (donky-change) :type 'end-of-buffer))))

(ert-deftest donky-change-no-region-at-end-of-buffer-errors ()
...
(ert-deftest donky-change-region-deletes-region ()
  "With an active region (not rectangle), deletes from mark to point.
    ...
    Expected: delete-region called with (1, 6), donky-enter-insert called."
...
(ert-deftest donky-change-region-enters-insert ()
...
(ert-deftest donky-change-region-skips-delete-char ()
...
(ert-deftest donky-change-region-mark-before-point ()
...
(ert-deftest donky-change-region-point-before-mark ()
...
(ert-deftest donky-change-region-single-char-region ()
...
(ert-deftest donky-change-rectangle-mode-calls-string-rectangle ()
...
(ert-deftest donky-change-rectangle-mode-skips-delete-region ()
...
(ert-deftest donky-change-rectangle-mode-enters-insert ()
  "In rectangle mode, still enters insert mode after string-rectangle.
    Expected: donky-enter-insert called."
  (let (entered)
    (with-temp-buffer
      (insert "hello\n")
      (goto-char 1)
...
(ert-deftest donky-change-rectangle-mode-falls-back-when-disabled ()
...
(ert-deftest donky-change-no-region-preserves-surrounding-text ()
...
(ert-deftest donky-change-region-deletes-correct-text ()
...
(ert-deftest donky-change-call-interactively-no-region ()
  "Can be called interactively without a region.
    Expected: no error, delete-char and donky-enter-insert execute."
  (let (entered)
    (with-temp-buffer
      (insert "hello\n")
      (goto-char 1)
      (cl-letf (((symbol-function 'use-region-p)
                ...
                ((symbol-function 'donky-enter-insert)
                 (lambda () (setq entered t))))
        (call-interactively #'donky-change))
      (should entered)
      (should (= (buffer-size) 5)))))

(ert-deftest donky-change-call-interactively-with-region ()
  "Can be called interactively with a region.
    Expected: no error, delete-region and donky-enter-insert execute."
  (let (entered)
    (with-temp-buffer
      (insert "hello world\n")
      (goto-char 6)
    ...
    ;;; donky-change-test.el ends here
