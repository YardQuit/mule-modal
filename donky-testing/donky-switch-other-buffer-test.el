;;; donky-switch-other-buffer-test.el --- Tests for donky-switch-other-buffer -*- lexical-binding: t; -*-
...
;; ===========================================================================
;; Section: donky-switch-other-buffer
;; Selector: (ert "donky-switch-other-buffer")
;;           runs ALL tests in this file
;; ===========================================================================

;;; --- Basic functionality (mocked) ---

(ert-deftest donky-switch-other-buffer-calls-other-buffer ()
...
Expected: other-buffer invoked, receives current-buffer."
  (let (other-arg)
    (cl-letf (((symbol-function 'other-buffer)
      ...
      (donky-switch-other-buffer))
    (should other-arg)
    (should (eq other-arg (current-buffer)))))

(ert-deftest donky-switch-other-buffer-calls-switch-to-buffer ()
...
(ert-deftest donky-switch-other-buffer-call-order ()
...
(ert-deftest donky-switch-other-buffer-other-buffer-called-once ()
...
(ert-deftest donky-switch-other-buffer-switch-to-buffer-called-once ()
...
(ert-deftest donky-switch-other-buffer-passes-current-buffer-explicitly ()
...
(ert-deftest donky-switch-other-buffer-switches-to-real-buffer ()
...
(ert-deftest donky-switch-other-buffer-switches-back-and-forth ()
...
Expected: switch-to-buffer receives the correct buffer on each call."
  (let ((results)
        (buf-a (generate-new-buffer "*donky-test-aa*"))
        (buf-b (generate-new-buffer "*donky-test-bb*")))
    (unwind-protect
        (progn
          (cl-letf (((symbol-function 'other-buffer)
                     (lambda (buf)
...
(ert-deftest donky-switch-other-buffer-single-buffer ()
...
(ert-deftest donky-switch-other-buffer-other-buffer-returns-nil ()
...
(ert-deftest donky-switch-other-buffer-call-interactively ()
...
Expected: no error, other-buffer and switch-to-buffer execute."
  (let (other-called switch-called)
    (cl-letf (((symbol-function 'other-buffer)
              ...
              ((symbol-function 'switch-to-buffer)
               (lambda (buf) (setq switch-called t))))
      (call-interactively #'donky-switch-other-buffer))
    (should other-called)
    (should switch-called)))

;;; --- Ignores prefix arg ---

(ert-deftest donky-switch-other-buffer-ignores-prefix-arg ()
...
;;; donky-switch-other-buffer-test.el ends here
